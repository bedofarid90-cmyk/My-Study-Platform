...
let currentUser = null, subjectsMeta = {}, allData = {}, userProgress = {};
...
async function loadDataFromCloud() {
    ...
    try {
        ...
        if (doc.exists) {
            const data = doc.data(); subjectsMeta = data.subjectsMeta || {}; allData = data.allData || {}; userProgress = data.userProgress || {};
...
function saveDataToCloud() { if (!currentUser) return; db.collection('users').doc(currentUser.uid).set({ subjectsMeta: subjectsMeta, allData: allData, userProgress: userProgress, lastUpdated: firebase.firestore.FieldValue.serverTimestamp() }, { merge: true }).catch(err => console.error(err)); }
...
function renderSubjects() {
    ...
    if (Object.keys(subjectsMeta).length === 0) return container.innerHTML = `<p style="text-align:center; width:100%; opacity:0.7;">${currentLang === 'ar' ? 'مفيش مواد لسه..' : 'No subjects yet..'}</p>`;
...
function openSubject(subKey, subName) { if (!currentUser) return showToast(t('js_demoReq'), "error"); currentSubjectKey = subKey; currentSubjectName = subName; document.getElementById('subjectTitle').innerText = subName; updateDashboardUI(); renderLectures(); showScreen('subjectScreen'); }
...
function openLecture(lecName) { currentLectureName = lecName; document.getElementById('lectureTitle').innerText = `${currentSubjectName} - ${lecName}`; document.getElementById('addQSubjectName').innerText = `(${lecName})`; let count = allData[currentSubjectKey][lecName].length; document.getElementById('lectureQCount1').innerText = `${currentLang==='ar'?'الأسئلة:':'Total:'} ${count}`; document.getElementById('lectureQCount2').innerText = `${t('timeLabel')} ${count} ${currentLang==='ar'?'دقيقة':'min'}`; showScreen('lectureScreen'); }

function initSubjectProgress(subjectKey) { if (!userProgress[subjectKey]) userProgress[subjectKey] = { total: 0, correct: 0, wrong: 0 }; }
function updateDashboardUI() { if (!currentSubjectKey) return; initSubjectProgress(currentSubjectKey); let prog = userProgress[currentSubjectKey]; document.getElementById('statTotal').innerText = prog.total; document.getElementById('statCorrect').innerText = prog.correct; document.getElementById('statWrong').innerText = prog.wrong; document.getElementById('statPercent').innerText = (prog.total === 0 ? 0 : Math.round((prog.correct / prog.total) * 100)) + '%'; }
...
function toggleAddMode(mode) { if(mode === 'single') { document.getElementById('singleAddMode').classList.remove('hidden'); document.getElementById('bulkAddMode').classList.add('hidden'); document.getElementById('btnSingleMode').style.backgroundColor = 'var(--primary-color)'; document.getElementById('btnBulkMode').style.backgroundColor = 'var(--secondary-color)'; } else { document.getElementById('singleAddMode').classList.add('hidden'); document.getElementById('bulkAddMode').classList.remove('hidden'); document.getElementById('btnSingleMode').style.backgroundColor = 'var(--secondary-color)'; document.getElementById('btnBulkMode').style.backgroundColor = 'var(--primary-color)'; } }
...
function generateAIPrompt() {
    const sourceText = document.getElementById('aiSourceText').value.trim(); const qCount = document.getElementById('aiQCount').value; const qType = document.getElementById('aiQType').value;
    if (!sourceText) return alert(currentLang==='ar'?"حط المادة العلمية الأول!":"Paste material first!");
    let typeInstruction = "";
    if (qType === "mcq") { typeInstruction = currentLang==='ar'?"كل الأسئلة تكون اختياري (MCQ). الصيغة:\n[السؤال]\n[الاختيار 1]\n[الاختيار 2]\n[الاختيار 3]\n[الاختيار 4]\n[الإجابة الصحيحة بالظبط]":"All questions MCQ. Format:\n[Question]\n[Opt 1]\n[Opt 2]\n[Opt 3]\n[Opt 4]\n[Exact Correct Answer]"; } 
    else if (qType === "tf") { typeInstruction = currentLang==='ar'?"كل الأسئلة صح وخطأ. الصيغة:\n[السؤال]\n[صح أو غلط] (اكتب كلمة 'صح' أو 'غلط')":"All questions True/False. Format:\n[Question]\n[صح or غلط] (Use exact Arabic words 'صح' or 'غلط')"; } 
    else { typeInstruction = currentLang==='ar'?"نص الأسئلة اختياري والنص صح وخطأ بنفس الصيغ المذكورة.":"Half MCQ, Half T/F using the exact formats."; }
    ...
    navigator.clipboard.writeText(prompt).then(() => { showToast(t('js_copied')); document.getElementById('aiActionButtons').style.display = 'flex'; document.getElementById('aiSourceText').value = ''; }).catch(err => alert('Copy failed!'));
}
...
function renderManageQuestions() {
    ...
    questions.forEach((q, index) => {
        ...
        container.innerHTML += `<div style="background: var(--input-bg); padding: 15px; border-radius: 10px; border: 1px solid var(--input-border); position: relative; direction: ${currentLang==='ar'?'rtl':'ltr'}; text-align: ${currentLang==='ar'?'right':'left'};"><div style="position: absolute; top: 10px; ${currentLang==='ar'?'left: 10px;':'right: 10px;'} direction: ltr;"><button onclick="editQuestion(${index})" class="btn" style="padding: 5px 10px; font-size: 14px;">✏️</button><button onclick="deleteQuestion(${index})" class="btn btn-danger" style="padding: 5px 10px; font-size: 14px;">🗑️</button></div><div style="${currentLang==='ar'?'padding-left':'padding-right'}: 150px;"><strong>Q${index + 1}:</strong> ${q.q}<div style="color: var(--success-color); margin-top: 5px;"><strong>Answer:</strong> ${q.a}</div>${optsHtml}</div></div>`;
    });
}
...
function loadQuestion() {
    ...
    if(currentQIndex < questionsList.length) {
        if (!isExamMode && document.getElementById('questionSelect')) { const qSelect = document.getElementById('questionSelect'); qSelect.innerHTML = ''; questionsList.forEach((q, i) => { let opt = document.createElement('option'); opt.value = i; opt.text = `Q ${i + 1}`; if (i === currentQIndex) opt.selected = true; qSelect.appendChild(opt); }); }
        document.getElementById('quizProgress').innerText = `Question ${currentQIndex + 1} of ${questionsList.length}`; const currentQ = questionsList[currentQIndex]; document.getElementById('questionText').innerText = currentQ.q;
        if (currentQ.type === "tf") { optionsDiv.innerHTML += `<button class="btn btn-option" onclick="checkAnswer('صح')">${currentLang==='ar'?'صح':'True'}</button><button class="btn btn-option" onclick="checkAnswer('غلط')">${currentLang==='ar'?'غلط':'False'}</button>`; } 
    ...
    } else { isExamMode ? endExam() : (document.getElementById('quizProgress').innerText = currentLang==='ar'?"تم الانتهاء":"Finished", document.getElementById('questionText').innerText = "🎉", optionsDiv.style.display = 'none', document.getElementById('normalNavControls').style.display = 'none', document.getElementById('externalAiControls').style.display = 'none'); }
}
...
function checkAnswer(userAnswer) {
    ...
    const feedbackDiv = document.getElementById('feedback'), controlsDiv = document.getElementById('controls'); document.getElementById('options').style.display = 'none'; feedbackDiv.style.display = 'block'; controlsDiv.style.display = 'flex'; controlsDiv.style.gap = '10px';
    if (userAnswer === currentQ.a) { userProgress[currentSubjectKey].correct++; playSound('correct'); feedbackDiv.className = 'feedback correct'; feedbackDiv.innerHTML = `✅ <strong>Correct!</strong> <br><br> ${currentQ.explanation || ''}`; document.getElementById('tryAgainBtn').style.display = 'none'; document.getElementById('nextBtn').style.display = 'inline-block'; } 
    else { userProgress[currentSubjectKey].wrong++; playSound('wrong'); feedbackDiv.className = 'feedback wrong'; feedbackDiv.innerHTML = `❌ <strong>Wrong!</strong> <br><br> ${currentQ.explanation || ''}`; document.getElementById('tryAgainBtn').style.display = 'inline-block'; document.getElementById('nextBtn').style.display = 'none'; }
    saveDataToCloud(); updateDashboardUI();
}
function endExam() { clearInterval(examTimerInterval); const totalQs = allData[currentSubjectKey][currentLectureName].length; const percent = Math.round((examScore / totalQs) * 100); if (percent === 100) triggerConfetti(); document.getElementById('finalScoreText').innerText = `${percent}%`; showScreen('examResultScreen', false); history.replaceState({ screen: 'examResultScreen', subjectKey: currentSubjectKey, subjectName: currentSubjectName, lectureName: currentLectureName, qIndex: currentQIndex, isExam: isExamMode }, '', '#examResultScreen'); }
